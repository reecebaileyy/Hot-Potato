...
export default function Play({ initalGameState, gen, price, maxSupply }: PlayProps): React.JSX.Element {
  ...
  const handlePotatoMinted = useCallback(async (logs: any[]) => {
    try {
      ...
      setRoundMints(parsedResults?.parsedResults?.totalMints || 0)
      setEvents((prev) => [...prev, `${player} just minted ${amount} hands`])
    } catch (error) {
      console.error('Error updating mints', error)
    }
  }, [_address, parsedResults?.parsedResults?.totalMints])
  ...
  const parsedResults = useMemo(() => {
    ...
    return {
      ...
      parsedResults?._potato_token: readResults[1] ? parseInt(readResults[1] as unknown as string, 10) : 0,
      _currentGeneration: readResults[2] ? parseInt(readResults[2] as unknown as string, 10) : 0,
      parsedResults?.totalMints: readResults[3] ? parseInt(readResults[3] as unknown as string, 10) : 0,
      parsedResults?._ownerAddress: readResults[4]?.toString(),
      allWinners: readResults[5] as `0x${string}`[] | undefined,
      parsedResults?._rewards: readResults[6] ? formatUnits(readResults[6] as unknown as bigint, 18) : '0',
      parsedResults?.totalWins: readResults[7] ? parseInt(readResults[7] as unknown as string, 10) : 0,
      parsedResults?.maxPerWallet: readResults[8] ? parseInt(readResults[8] as unknown as string, 10) : 0,
      parsedResults?.isTokenActive: readResults[9] as unknown as boolean,
      parsedResults?.ownerOf: readResults[10]?.toString(),
    }
  }, [readResults])
  ...
  const _potatoOwner = useMemo(() => getPotatoOwner?.toString(), [getPotatoOwner]);
  ...
  const _activeTokens = useMemo(() => getActiveTokens as unknown as number[], [getActiveTokens]);
  ...
  const activeAddresses = useMemo(() => _activeAddresses as unknown as `0x${string}`[], [_activeAddresses]);
  ...
  function parsedResults?.maxPerWalletToast() {
  ...
  // --- Loading states ---
  const isLoading = loadingReadResults || loadingExplosionTime || loadingHGallOfFame || loadingHasPotato || loadingPotatoOwner || loadingActiveTokens || loadingActiveAddresses || loadingSuccessfulPasses || loadingActiveTokenCount || loadingActiveTokenIds || balanceLoading
  ...
  // --- Optimized effects ---
  useEffect(() => {
    if (parsedResults?.parsedResults?._potato_token) {
      setPotatoTokenId(parsedResults.parsedResults?._potato_token);
    }
    if (explosionTime) {
      setRemainingTime(explosionTime);
    }
  }, [parsedResults?.parsedResults?._potato_token, explosionTime]);
    ...
    <>
      ...
      <div className={`${darkMode ? 'darkmode bg-fixed text-white min-h-screen font-darumadrop' : 'normal bg-fixed min-h-screen font-darumadrop'}`}>
        ...
        <div className="p-4 sm:flex sm:flex-col md:flex md:flex-col lg:flex lg:flex-col grid grid-cols-8 gap-4 justify-center items-center">
          <div className={`w-full flex flex-col items-center col-start-1 col-end-3 md:w-2/3 lg:w-1/2 shadow rounded-xl p-4 mb-8 overflow-y-auto hide-scrollbar ${getGameState !== "Paused" && getGameState !== "Queued" && address ? "max-h-96" : ""} ${darkMode ? 'bg-black text-white' : 'bg-white text-black'}`}>
            {isLoading ? (
              ...
              getGameState == "Playing" || getGameState == "Final Stage" ?
                <>
                  {isWinner && Number(parsedResults?._rewards) != 0 &&
                    <button className={`${darkMode ? 'w-1/5 hover:bg-white hover:text-black justify-center items-center md:w-2/3 lg:w-1/2 bg-black shadow rounded-xl' : "w-1/2 leading-8 hover:bg-black hover:text-white col-start-2 col-span-6 justify-center items-center md:w-2/3 lg:w-1/2 bg-white shadow rounded-xl"}`}
                      onClick={() => {
                        if (!_address) {
                          noAddressToast();
                        } else if (Number(parsedResults?._rewards) == 0) {
                          noRewardsToast();
                          ...
                          <button className={`${darkMode ? 'w-1/2 hover:bg-white hover:text-black justify-center items-center md:w-2/3 lg:w-1/2 bg-black shadow rounded-xl' : "w-1/2 leading-8 hover:bg-black hover:text-white col-start-2 col-span-6 justify-center items-center md:w-2/3 lg:w-1/2 bg-white shadow rounded-xl"}`}
                            onClick={() => {
                              if (!_address) {
                                ...
                                console.log('claiming rewards', parsedResults?._rewards, Number(parsedResults?._rewards));
                                writeClaim(claimSim.request);
                              }
                            }}
                          >Claim Rewards</button>
            ...
            {getGameState == "Playing" || getGameState == "Final Stage" ?
              <>
                <h1 className={`text-4xl font-extrabold underline text-center mb-4 text-transparent bg-clip-text ${darkMode ? 'bg-gradient-to-br from-amber-800 to-red-800' : 'bg-gradient-to-b from-yellow-400 to-red-500'}`}>
                  {!parsedResults?._potato_token ? `Loading...` : `Token #${parsedResults?._potato_token} has the potato`}
                </h1>
                {loadingHasPotato ? (
                  ...
                  getGameState == "Ended" ? (
                    ...
                    getGameState == "Minting" && (
                      <>
                        ...
                        <p className={`text-lg text-center mb-4 ${darkMode ? 'text-white' : 'text-black'}`}>MAX PER WALLET: <span className={`text-transparent bg-clip-text ${darkMode ? 'bg-gradient-to-br from-amber-800 to-red-800' : 'bg-gradient-to-b from-yellow-400 to-red-500'}`}>{parsedResults?.maxPerWallet}</span></p>
                        ...
                        {_address ?
                          <>
                            ...
                            <button className={`mt-4 w-1/2 ${darkMode ? 'bg-gray-800 hover:bg-gradient-to-br from-amber-800 to-red-800' : 'bg-black'} hover:bg-gradient-to-b from-yellow-400 to-red-500 text-white px-4 py-3 rounded-lg shadow-lg text-lg font-bold transition-all duration-500 ease-in-out transform hover:scale-110`}
                              onClick={() => {
                                if (!_address) {
                                  ...
                                  //   mintOneToast();
                                  // }
                                  // else if (activeTokensCount + parseInt(mintAmount) > parsedResults?.maxPerWallet) {
                                  //   parsedResults?.maxPerWalletToast();
                                }
                            ...
                            >Join Round!</button>
                            <p className={`text-lg text-center mb-4 ${darkMode ? 'text-white' : 'text-black'}`}>{parsedResults?.totalMints}/{maxSupply} MINTED</p>
                          </>
                          :
                          <>
                            <p className={`text-3xl text-center mb-4 ${darkMode ? 'text-white' : 'text-black'}`}>{parsedResults?.totalMints}/{maxSupply} MINTED</p>
            ...
            {!_address ?
              ...
              getGameState == "Playing" || getGameState == "Final Stage" ?
                <>
                  ...
                  <p className={`text-sm text-center mb-2 ${darkMode ? 'text-white' : 'text-black'}`}>
                    Total Wins: {parsedResults?.totalWins}
                  </p>
                    ...
                    <button className={`mt-4 w-full ${darkMode ? 'bg-gray-800 hover:hover:bg-gradient-to-br from-amber-800 to-red-800' : 'bg-black'} hover:bg-gradient-to-b from-yellow-400 to-red-500 text-white px-4 py-2 rounded-lg shadow`}
                      onClick={() => {
                        ...
                        if (!_address) {
                          ...
                          tokenInactiveToast();
                        } else if (_address == parsedResults?.ownerOf) {
                    ...
                    <div key={index} className="border rounded-lg p-2 text-center justify-center items-center flex flex-col">
                      <Suspense fallback={<SkeletonCard />}>
                        <TokenImage
                          tokenId={tokenId}
                          ABI={ABI}
                          potatoTokenId={parsedResults?.parsedResults?._potato_token || 0}
                          shouldRefresh={shouldRefresh}
          ...
          {_address === parsedResults?._ownerAddress &&
